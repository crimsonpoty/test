...
#define SYSLOG_(fmt, args...) do{syslog(LOG_INFO, fmt, ##args);}while(0)
...
#define M_(fmt,args...) do{TIME_PRINT; fprintf(stderr, "{%8x} "fmt,(int)pthread_self(),##args);fprintf(stderr, " || %s[L:%d]\n",__PRETTY_FUNCTION__, __LINE__);}while(0)
#define TODO(fmt,args...) do{TIME_PRINT; fprintf(stderr, "{%8x} "fmt,(int)pthread_self(),##args);fprintf(stderr, " || %s[L:%d]\n",__PRETTY_FUNCTION__, __LINE__);}while(0)
#if 0
#define m_(fmt,args...) do{TIME_PRINT; fprintf(stderr, "{%8x} "fmt,(int)pthread_self(),##args);fprintf(stderr, " || %s[L:%d]\n",__PRETTY_FUNCTION__, __LINE__);}while(0)
#else
#define m_(fmt,args...) do{TIME_PRINT; fprintf(stderr, "{%8x} "fmt,(int)pthread_self(),##args);fprintf(stderr, "\n");}while(0)
#endif
